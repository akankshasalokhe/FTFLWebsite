...
//   setLoading(true);
//   axios.get(`https://landing-page-yclw.vercel.app/api/blog/${id}`)
//     .then(res => {
...
//       <Head>
//         <title>{blogData.title} | Muze Blog</title>
...
//         {/* <Link href={`/blog/${blogData.category.toLowerCase()}`} className="nav-link">{blogData.category}</Link> */}
//         <span className="nav-separator"> / </span>
...
//                 <span className="date">{blogData.createdAt}</span>
...
//             {/* <div 
//               className="content"
//               dangerouslySetInnerHTML={{ __html: blogData.items.itemDescription }}
//             /> */}
//             {blogData.items?.map((item, index) => (
//               <div key={index}>
//                 <div
//                   className="content"
//                   dangerouslySetInnerHTML={{ __html: item.itemDescription?.join('<br/>') }}
//                 />
//               </div>
//             ))}
...
export default function BlogDetail() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            setLoading(true);
            try {
                const { data: currentData } = await axios.get(`https://landing-page-yclw.vercel.app/api/blog/${id}`);
                setBlogData(currentData.data);
    ...
    if (error) {
        return (
            <div className="container text-center py-20">
                ...
                <button onClick={() => router.push('/blog')} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded">
                    Go to Blog List
                </button>
            </div>
        );
    ...
    if (!blogData) {
        return (
            <div className="container text-center py-20">
                <p>No blog post found for ID: {id}</p>
                <button onClick={() => router.push('/blog')} className="mt-4 px-4 py-2 bg-blue-500 text-white rounded">
                    Go to Blog List
                </button>
            </div>
            ...
            <div className="layout">
                <main className="main-content">
                    ...
                    <article className="content-section">
                        ...
                        <h1 className="title">{blogData.title}</h1>
                        <p className="excerpt">{blogData.description}</p>
