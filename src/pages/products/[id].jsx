...
export default function ProductDetail() {
  ...
  useEffect(() => {
    axios
      .get(`https://landing-page-yclw.vercel.app/api/product/${id}`)
      .then((res) => {
        console.log("API response:", res.data.data); // ✅ log here
        setProductData(res.data.data);
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900">
      <Head>
        <title>{productData.title} — Product Details</title>
        <style>{`
          @keyframes fadeInUp {
            from {
              opacity: 0;
          ...
          .gradient-border::before {
            ...
            right: -2px;
            bottom: -2px;
            background: linear-gradient(45deg, ${productData.color}, ${productData.accentColor});
            border-radius: 1.1rem;
            z-index: -1;
            opacity: 0.7;
          }
      ...
      <header className="p-4 shadow bg-white flex items-center justify-between sticky top-0 z-50 animate-fadeIn">
        ...
        <h1 className="text-xl font-semibold text-transparent bg-clip-text" style={{ backgroundImage: `linear-gradient(45deg, ${productData.color}, ${productData.accentColor})` }}>
          {productData.title}
        </h1>
      </header>
      ...
      {isLoading && (
        <div className="fixed inset-0 bg-white bg-opacity-80 z-50 flex items-center justify-center">
          <div className="flex flex-col items-center">
            <div className="w-16 h-16 border-4 border-indigo-600 border-t-transparent rounded-full animate-spin"></div>
            <p className="mt-4 text-gray-600">Loading {productData.title}...</p>
          </div>
        </div>
      )}
      ...
      <section className="max-w-6xl mx-auto p-6 animate-fadeIn" style={{ animationDelay: "0.2s" }}>
        {/* <div className="gradient-border">
          <Swiper
            ...
            className="rounded-2xl overflow-hidden shadow-lg border-2 border-white"
          >
            {productData.images.map((id, index) => (
              <SwiperSlide key={id}>
                <div className={`w-full h-80 md:h-96 bg-gray-200 rounded-2xl overflow-hidden ${isLoading ? 'shimmer' : ''}`}>
                  {!isLoading && (
                    <img
        ...
        <div className="gradient-border">
          <Swiper
            ...
            className="rounded-2xl overflow-hidden shadow-lg border-2 border-white"
          >
            {productData.screenshot?.map((shot, index) => (
              <SwiperSlide key={shot._id}>
                <div
                  className={`w-full h-80 md:h-96 bg-gray-200 rounded-2xl overflow-hidden ${isLoading ? "shimmer" : ""
                    }`}
                >
                  {!isLoading && (
                    <img
                      src={shot.screenshotImage}
                      alt={`${productData.title} - Screenshot ${index + 1}`}
                      className="w-full h-full object-cover transition-transform duration-700 hover:scale-105"
                    />
                  )}
                </div>
        ...
        <Swiper
          onSwiper={setThumbsSwiper}
          spaceBetween={10}
          slidesPerView={Math.min(productData.screenshot?.length || 0, 5)}
          freeMode={true}
          watchSlidesProgress
          className="mt-4 animate-fadeIn"
          style={{ animationDelay: "0.4s" }}
        >
          {productData.screenshot?.map((shot) => (
            <SwiperSlide key={shot._id} className="cursor-pointer">
              <div className="rounded-lg overflow-hidden border-2 border-transparent transition-all duration-300 hover:border-indigo-500 hover:scale-105">
                <img
                  src={shot.screenshotImage}
                  alt="Thumbnail"
        ...
        <div className="flex justify-center mt-6 animate-fadeIn" style={{ animationDelay: "0.6s" }}>
          <a
            href={productData.videoFile}
            target="_blank"
            rel="noopener noreferrer"
            className="px-6 py-3 rounded-xl text-white transition-all duration-300 hover-lift flex items-center gap-2"
            style={{ background: `linear-gradient(45deg, ${productData.color}, ${productData.accentColor})` }}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM9.555 7.168A1 1 0 008 8v4a1 1 0 001.555.832l3-2a1 1 0 000-1.664l-3-2z" clipRule="evenodd" />
            </svg>
    ...
    {productData.keyFeatures?.length > 0 ? (
      productData.keyFeatures.map((feature, index) => (
        <div
          ...
          style={{ animationDelay: `${0.1 * index}s` }}
        >
          {/* Use featureIcon as an image */}
          <img
            src={feature.featureIcon}
            alt={feature.featureTitle}
            className="w-16 h-16 mb-4 object-contain"
          />
          <h3 className="text-xl font-semibold mb-2 text-gray-800">{feature.featureTitle}</h3>
          <p className="text-gray-600">{feature.featureDescription}</p>
        </div>
      ))
    ) : (
      <p className="text-center text-gray-500 col-span-full">No features available.</p>
    )}
  </div>
</section>
      ...
      <section
        className="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow-md mt-8 animate-fadeInUp"
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {productData.productControls?.map((control, index) => {
            // Choose icon based on position
            const icons = ["💡", "ℹ️", "⚙️"];
            const icon = icons[index] || "ℹ️";

            return (
              <div key={control._id} className="text-center">
                {/* Icon */}
                <div
                  className="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center text-white text-2xl"
                  style={{
                    background: `linear-gradient(45deg, #4ECDC4, #2BB8AF)`,
                  }}
                ...
                <h3 className="text-xl font-semibold mb-3 text-gray-800">
                  {control.productTitle}
                </h3>

                {/* Description */}
                <p className="text-gray-600">{control.productDescription}</p>
              </div>
            );
          })}
        </div>
      ...
      {/* <section className="max-w-7xl mx-auto p-6 mt-8 animate-fadeInUp" style={{ animationDelay: "0.6s" }}>
        ...
        <div className="flex flex-wrap justify-center gap-6">
          {productData.downloadLinks.playStore && (
            <a
              href={product.downloadLinks.playStore}
              className="flex items-center justify-center gap-3 px-6 py-4 bg-gray-800 text-white rounded-xl hover-lift transition-all duration-300 w-full sm:w-auto"
            >
          ...
          {productData.downloadLinks.appStore && (
            <a
              href={productData.downloadLinks.appStore}
              className="flex items-center justify-center gap-3 px-6 py-4 bg-gray-800 text-white rounded-xl hover-lift transition-all duration-300 w-full sm:w-auto"
            >
              <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
                <path d="M17.05 12.04C17.03 9.53 19.18 8.05 19.25 8C17.95 6.06 15.87 5.85 15.1 5.83C13.55 5.66 12.06 6.84 11.24 6.84C10.4 6.84 9.14 5.86 7.81 5.88C6.06 5.9 4.48 6.83 3.62 8.4C1.8 11.45 3.06 16.13 4.79 18.58C5.62 19.75 6.62 21.05 7.87 20.98C9.08 20.91 9.57 20.14 11.04 20.14C12.49 20.14 12.96 20.98 14.22 20.95C15.52 20.91 16.39 19.75 17.2 18.58C18.16 17.25 18.56 15.96 18.58 15.9C18.53 15.88 15.95 14.75 15.99 11.74L17.05 12.04ZM15.3 4.1C15.95 3.3 16.41 2.16 16.25 1C15.25 1.05 14.07 1.66 13.38 2.46C12.78 3.16 12.21 4.33 12.4 5.45C13.5 5.55 14.6 4.95 15.3 4.1Z" />
          ...
          {productData.downloadLinks.web && (
            <a
              href={productData.downloadLinks.web}
              className="flex items-center justify-center gap-3 px-6 py-4 text-white rounded-xl hover-lift transition-all duration-300 w-full sm:w-auto"
              style={{ background: `linear-gradient(45deg, ${productData.color}, ${productData.accentColor})` }}
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8" viewBox="0 0 20 20" fill="currentColor">
...
<section
  className="max-w-7xl mx-auto p-6 mt-8 animate-fadeInUp"
...
>
  ...
  {productData.category === "App" ? (
    <div className="flex flex-wrap justify-center gap-6">
      {productData.googleStoreLink && (
        <a
          href={productData.googleStoreLink}
          className="flex items-center justify-center gap-3 px-6 py-4 bg-gray-800 text-white rounded-xl hover-lift transition-all duration-300 w-full sm:w-auto"
        >
          <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
            <path d="M3.609 1.814L13.792 12 3.609 22.186a.996.996 0 0 1-.61-.92V2.734a1 1 0 0 1 .61-.92zm10.781 10.186l-2.522 2.521 7.874 7.875c.138.137.302.25.486.333l-5.838-10.729zM12 13.208l-9.186 9.187a.995.995 0 0 1-.92.61h18.734a1 1 0 0 1 .92-.61L12 13.208zM5.67 3.331L12 10.792l2.522-2.521-7.875-7.875a1.001 1.001 0 0 0-.486.333z" />
      ...
      {productData.appleStoreLink && (
        <a
          href={productData.appleStoreLink}
          className="flex items-center justify-center gap-3 px-6 py-4 bg-gray-800 text-white rounded-xl hover-lift transition-all duration-300 w-full sm:w-auto"
        >
          <svg className="w-8 h-8" viewBox="0 0 24 24" fill="currentColor">
            <path d="M17.05 12.04C17.03 9.53 19.18 8.05 19.25 8C17.95 6.06 15.87 5.85 15.1 5.83C13.55 5.66 12.06 6.84 11.24 6.84C10.4 6.84 9.14 5.86 7.81 5.88C6.06 5.9 4.48 6.83 3.62 8.4C1.8 11.45 3.06 16.13 4.79 18.58C5.62 19.75 6.62 21.05 7.87 20.98C9.08 20.91 9.57 20.14 11.04 20.14C12.49 20.14 12.96 20.98 14.22 20.95C15.52 20.91 16.39 19.75 17.2 18.58C18.16 17.25 18.56 15.96 18.58 15.9C18.53 15.88 15.95 14.75 15.99 11.74L17.05 12.04ZM15.3 4.1C15.95 3.3 16.41 2.16 16.25 1C15.25 1.05 14.07 1.66 13.38 2.46C12.78 3.16 12.21 4.33 12.4 5.45C13.5 5.55 14.6 4.95 15.3 4.1Z" />
    ...
    <div className="flex justify-center">
      ...
      {/* Sales Contact Section */}
{( productData.contact || productData.emailLink) && (
  <section
    className="max-w-7xl mx-auto p-6 mt-8 bg-white rounded-xl shadow-md animate-fadeInUp"
  ...
  >
    ...
    <div className="grid md:grid-cols-2 gap-8">
      <div className="text-center p-6 rounded-xl bg-gray-50 hover-lift transition-all duration-300">
        <div
          className="w-16 h-16 mx-auto mb-4 rounded-full flex items-center justify-center text-white text-2xl"
          style={{ background: `linear-gradient(45deg, #4ECDC4, #2BB8AF)` }}
        >
          ✉️
        </div>
        <h3 className="text-xl font-semibold mb-3 text-gray-800">Email Us</h3>
        <a
          href={`mailto:${ productData.emailLink}`}
          className="text-lg text-indigo-600 hover:text-indigo-800 transition-colors"
        >
          {productData.emailLink}
        </a>
      </div>
...
{productData.faqs && productData.faqs.length > 0 && (
  ...
  >
    ...
    <div className="space-y-4">
      ...
      {/* Footer */}
      <footer className="max-w-7xl mx-auto p-6 mt-12 text-center text-gray-500 border-t pt-6 animate-fadeIn" style={{ animationDelay: "0.9s" }}>
        <p>© {new Date().getFullYear()} {productData.title}. All rights reserved.</p>
      </footer>
    </div>
  );
}